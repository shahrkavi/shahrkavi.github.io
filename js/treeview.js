...
(function () { // Anonymous IIFE for encapsulation
    ...
    class Treeview {
        ...
        /**
         * Sets button content (icon, text, or both) and title based on buttonStyle option.
        ...
        _setButtonContent(button, icon, text, title, biClass) {
            ...
            if (style === 'bootstrapIcons') {
                ...
                textSpan.classList.add('treeview-btn-text');
        ...
        _renderTree(data, parentElement) {
            ...
            data.forEach(node => {
                ...
                // 3. Add checkbox if enabled and node is selectable, right after the expander/placeholder
                if (this.options.checkboxSelectionEnabled && isNodeSelectable) {
                ...
                if (node.children && node.children.length > 0) {
                    ...
                    expanderOrPlaceholder.addEventListener('click', (event) => {
                        const childUl = li.querySelector('ul');
                        if (childUl) {
                            // Accordion mode: collapse all siblings and their descendants
                            if (this.options.accordionMode) {
                                const parentUl = li.parentElement;
                                if (parentUl && parentUl.parentElement) {
                                    parentUl.parentElement.querySelectorAll(':scope > li.expanded').forEach(siblingLi => {
                                        if (siblingLi !== li) {
                                            siblingLi.classList.remove('expanded');
                                            const siblingExpander = siblingLi.querySelector('.treeview-expander');
                                            if (siblingExpander) siblingExpander.textContent = '+';
                                            const siblingUl = siblingLi.querySelector('ul');
                                            if (siblingUl) {
        ...
        /**
         * Programmatically selects or deselects all selectable nodes in the tree.
         * Requires multiSelectEnabled to be true and cascadeSelectChildren to be false.
         * @param {boolean} [shouldSelect=true] True to select all nodes, false to deselect all.
         */
        selectAll(shouldSelect = true) {
            if (!this.options.nodeSelectionEnabled) {
                console.warn("Quercus.js: Node selection is disabled, cannot select/deselect all nodes.");
                return;
            }
            if (!this.options.multiSelectEnabled) {
                console.warn("Quercus.js: selectAll() requires multi-select to be enabled.");
                return;
            }
            if (this.options.cascadeSelectChildren) {
                console.warn("Quercus.js: selectAll() is not applicable when cascading selection is enabled.");
                return;
            }
